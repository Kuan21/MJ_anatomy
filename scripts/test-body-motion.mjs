...
for(const region of ['head','leftLeg','rightLeg']){
 ...
 for(const [name,pose] of Object.entries(poses)){
  ...
  for(const {p,b,positions,indices,skin} of posedParts){
   ...
   if(skin)for(let i=0;i<positions.length;i+=3)if(body.bodyWeights(rig,positions[i+1])[0]===1)assert.deepEqual([...out.slice(i,i+3)],[...positions.slice(i,i+3)],`${p.name} fixed origin`);
