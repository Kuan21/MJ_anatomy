  ...
  const animate=()=>{
   ...
   if(changed||moving||lastExtent<0){
    ...
    atlas.parts.forEach((p,i)=>{
     ...
     markerPositions.set(data[i*4+3]>.5?[c.x+dx,c.y+dy,c.z+dz]:[10000,10000,10000],i*3);const mesh=pickers[i];if(mesh){if(transform){const q=new T.Quaternion(...transform.quaternion);const pivot=new T.Vector3(...transform.pivot);mesh.quaternion.copy(q);mesh.position.copy(pivot).sub(pivot.clone().applyQuaternion(q)).add(new T.Vector3(dx,dy,dz));}else{mesh.quaternion.identity();mesh.position.set(dx,dy,dz);}mesh.updateMatrix();mesh.updateMatrixWorld(true);}
