...
export function makeSurfaceConstraints(base:Float32Array,triangles:ArrayLike<number>,skin:SkinBinding):SurfaceConstraints{
 const edges:number[]=[],lengths:number[]=[],seen=new Set<string>(),mobility=new Float32Array(base.length/3);
 ...
 for(let t=0;t<triangles.length;t+=3)for(let e=0;e<3;e++){
  const a=Math.min(triangles[t+e],triangles[t+(e+1)%3]),b=Math.max(triangles[t+e],triangles[t+(e+1)%3]),key=`${a}:${b}`;
